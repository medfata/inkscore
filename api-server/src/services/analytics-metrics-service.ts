...
export async function getCowswapSwaps(walletLower: string): Promise<CowSwapResult> {
  ...
  const cowLcKey = `long:analytics:cowswap_swaps:${walletLower}`;
  const cowLc = getLongCache<CowSwapResult>(cowLcKey, COWSWAP_LONG_CACHE_TTL);
  if (cowLc) {
    return cowLc;
  }
  return await withInflight<CowSwapResult>(cowLcKey, async (): Promise<CowSwapResult> => {
    try {
      let allOrders: any[] = [];
      let offset = 0;
