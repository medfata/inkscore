...
// Stablecoins are marked with usdPegged: true (1:1 with USD)
// Other tokens will use CoinGecko or DEX for price lookup
const KNOWN_TOKENS: Record<string, { symbol: string; decimals: number; usdPegged?: boolean; coingeckoId?: string }> = {
  ...
  // KBtc (Bitcoin-pegged token)
  '0x73e0c0d45e048d25fc26fa3159b0aa04bfa4db98': { symbol: 'KBtc', decimals: 8, coingeckoId: 'bitcoin' },
  // Add more tokens as needed - they will be fetched dynamically if not here
};

// Functions where the USD value needs to be extracted from input parameters
// For borrow/supply: asset (index 0) tells us the token, amount varies by function
...
export class AnalyticsService {
  ...
  private async getTokenPriceUsd(tokenAddress: string, ethPrice: number): Promise<number> {
    ...
    if (price === 0) {
      ...
      if (symbol.includes('USD') || symbol.includes('DAI') || symbol.includes('FRAX')) {
        ...
        // For unknown tokens, we can't determine price - return 0
        // In production, you'd want to integrate with a DEX price oracle
  ...
  private async queryMetricForWallet(
    ...
    for (const row of rows) {
      ...
      // For functions where value is in input params, we need to calculate differently
      ...
      if (needsInputData && inputDataUsdValues.size > 0) {
        // Get the sum of USD values from input data for this function
        ...
        // For borrow functions, the eth_total from DB is 0, so we use the USD value directly
        // Convert USD back to "ETH equivalent" for consistent handling
